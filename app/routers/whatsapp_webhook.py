...
from app.services.booking_confirm import finalize_booking_confirmation
...
_CONFIRM_WORDS = {"confirm", "confirmed", "paid", "payment"}
...
async def _try_admin_command(text: str, db) -> bool:
    """Handle admin WhatsApp commands (payment confirm or discount decisions)."""
    ...
    action_words = _APPROVE_WORDS | _REJECT_WORDS | _CONFIRM_WORDS
    ref = next(
    ...
    if confirm and not (approve or reject):
        ...
        if ref:
            ...
            await send_whatsapp_text(
                admin_number,
                f"There are {len(pending)} bookings awaiting payment. "
                f"Reply e.g. 'CONFIRM {pending[0].id[:8].upper()}'.\nPending: {refs}",
            )
            return True
        ...
        await send_whatsapp_text(
            admin_number,
            f"Booking {target.id[:8].upper()} confirmed. The guest has been notified.",
        )
        return True
